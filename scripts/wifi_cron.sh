...
#  1) eth1 (powerline) dod -> flush.
#  2) wlan0 mangler/uten IP -> S51wifi start.
#  3) wlan0 har IP men naar IKKE gateway (thrashing mellom IOT-AP) -> S51wifi start.
#  4) wlan0 OK -> gratuitous ARP.
#  5) MQTT-helse: stuck CLOSE_WAIT uten ESTABLISHED -> restart streamer straks;
#     ellers broker pingbar men ingen ESTABLISHED i 2 min -> restart.
IFACE=wlan0
GW=10.0.0.138
...
if ping -c 2 -W 2 -I $IFACE $GW >/dev/null 2>&1; then rm -f "$WFAIL"
else
  w=$(cat "$WFAIL" 2>/dev/null||echo 0); w=$((w+1)); echo $w > "$WFAIL"
  if [ $w -ge 2 ]; then
    log "wlan0 naar ikke gateway (thrashing) -> S51wifi start"; /etc/init.d/S51wifi start >/dev/null 2>&1; rm -f "$WFAIL"; exit 0
  fi
fi
...
[ -z "$PORT" ] && PORT=1883
EST=$(netstat -tn 2>/dev/null | grep -c "$BROKER:$PORT.*ESTABLISHED")
CW=$(netstat -tn 2>/dev/null | grep -c "$BROKER:$PORT.*CLOSE_WAIT")
restart_streamer() { log "$1 -> restart mqtt-streamer"; P=$(ps|grep "[m]qtt-streamer"|awk "{print \$1}"); [ -n "$P" ] && kill $P 2>/dev/null; rm -f "$MQF"; }
if [ "$EST" -ge 1 ]; then
  rm -f "$MQF"
elif [ "$CW" -ge 1 ]; then
  restart_streamer "MQTT stuck CLOSE_WAIT uten ESTABLISHED"
elif ping -c 1 -W 2 "$BROKER" >/dev/null 2>&1; then
  m=$(cat "$MQF" 2>/dev/null||echo 0); m=$((m+1)); echo $m > "$MQF"
  [ $m -ge 2 ] && restart_streamer "MQTT ingen forbindelse 2 min"
fi
